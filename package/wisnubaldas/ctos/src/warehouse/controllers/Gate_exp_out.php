...
class Gate_exp_out extends REST_Controller {
	...
	function index_get() {
        ...
        if ($id_kms == '') {
			$data = $this->db4->get('get_exp_out')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db4->where('id_kms', $id_kms);
        $update = $this->db4->update('get_exp_out', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db4->insert('get_exp_out', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
