...
class Sysmaster_model extends CI_Model 
{
	...
	public function list_route($airlinescode,$flightno)
	{
		$this->db2->where('airlinescode = ',$airlinescode);
		$this->db2->where('flightno = ',$flightno);
        $query = $this->db2->get('m_route');
        return $query->result();
	...
	public function list_Nama_angkut($airlinescode=null)
	{
		$this->db2->where('void=0');
		if(!is_null($airlinescode))$this->db2->where('airlinescode =',$airlinescode);
	...
	public function list_m_kd_dok_inout($keterangan)
	{
		$this->db2->select('*');
		$this->db2->where('keterangan=',$keterangan);	
        $query = $this->db2->get('m_kd_dok_inout');
        return $query->result();
	}
