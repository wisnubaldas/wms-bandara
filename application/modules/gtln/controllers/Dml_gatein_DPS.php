...
class Dml_gatein_DPS extends REST_Controller {
	...
	function index_get() {
        ...
        if ($id_kms == '') {
			$data = $this->db3->get('get_imp_in_dps')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db3->where('id_kms', $id_kms);
        $update = $this->db3->update('get_imp_in_dps', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db3->insert('get_imp_in_dps', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
