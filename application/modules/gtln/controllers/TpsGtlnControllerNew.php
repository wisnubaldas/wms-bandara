...
class TpsGtlnControllerNew extends MX_Controller
{
    public $wsdl = 'https://tpsonline.beacukai.go.id/tps/service.asmx?WSDL';
    public $setting;
    private $client;
    public $username = "GATR";
    ...
    public function __construct()
    {
        ...
        $this->tps->username = "GATR";
        $this->tps->password = "123456";
        $this->setting = array(       
    ...
    private function ref_num($num)
    {
        ...
        $refnum = "GATR".date('ymd').str_pad($num, 6,'0',STR_PAD_LEFT);
        return $refnum;
    }
    ...
    {	
        $query  =   'select * from tpsonline_gtln.get_imp_in where
			        flag_transfer=0  and kode_kantor = "050100" order by id_kms desc limit 100';
    ...
    public function impor_gate_out(){
        $query  =   'select * from tpsonline_gtln.get_imp_out where
			        flag_transfer=0  and kd_tps like "GATR" order by id_kms desc limit 100';
        $data   = $this->dbread->query($query)->result_array();
        //print_r($data);	   
        if(!$data)
        {
	...
	function looping_gate_out(){		
		...
		$file = fopen("lock_gateout_cgk.txt","w+");
        ...
        function looping_gate_in(){		
            $time_start = microtime(true);
            error_reporting('E_NONE');
            $file = fopen("lock_gatein_cgk.txt","w+");
            if (flock($file,LOCK_EX)){
                fwrite($file,"Write something too");
                for($j=0;$j<100;$j++){
                    echo "loop ke ".$j." ";
                    $data=$this->impor_gate_in();
                    sleep(1);
                    $time_end = microtime(true);
                    $time = $time_end - $time_start;
                    if($time>=65){
	...
	public function test_ipung(){
	   $hawb    = $this->input->get('hawb');
       $query   = 'select * from tpsonline_gtln.get_imp_out where
			        no_bl_awb like "'.$hawb.'" order by id_kms desc limit 1';
