...
class Import_model extends CI_Model
{
    ...
    {
        parent::__construct();			
		$this->db7 = $this->load->database('backup_rdwarehouse_jkt_read', TRUE);	
	...
	public function list_ATA_breakdowndetail($token,$MasterAWB=null)
	{
		$this->db7->distinct();
		$this->db7->select('*');
		$this->db7->where('void=0');
	...
	public function list_imp_location_mps_hostAWB($token,$hostAWB=null)
	{
		$this->db7->distinct();
		$this->db7->select('*');
		if(!is_null($hostAWB))$this->db7->where('hostAWB',$hostAWB);
