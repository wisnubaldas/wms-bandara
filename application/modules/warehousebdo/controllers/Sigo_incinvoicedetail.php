...
class Sigo_impinvoicedetail extends REST_Controller {
	...
	function index_get() {
        ...
        if ($InvoiceNumber == '') {
			$data = $this->db->get('sigo_inc_invoicedetail')->result();            
        } else {
	...
	'ProofNumber'	
	'MasterAWB'	
	'Pieces	CAW	Netto	WarehouseFee	AssistancyFee	CoolRoomFee	AirConditioningFee	ColdStorageFee	StrongRoomFee	DangerousRoomFee	OtherFee	AirportContriFee	OverStay	void	token
        ...
        $update = $this->db->update('sigo_inc_invoicedetail', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(	
		 ...
		 $insert = $this->db->insert('sigo_inc_invoicedetail', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
