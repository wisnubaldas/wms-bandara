...
class Whmaster_model extends CI_Model
{
    ...
    function __construct()
    {
		...
		$this->db2 = $this->load->database('rdlogin', TRUE);
    }
	...
	public function daftar_airlines($WHcode,$TwoLetterCode=null)
	{
		$this->db7->distinct();
		$this->db7->select('*');	
		if(!is_null($WHcode))$this->db7->where('KodeGudangByCustom=',$WHcode);
	...
	public function kodegudang($databaseCode,$warehouseCode)
	{
		$this->db2->distinct();
		$this->db2->select('*');		
		$this->db2->where('DatabaseCode',$databaseCode);				
		$this->db2->where('warehouseCode',$warehouseCode);
		$query = $this->db2->get('masterdatabase');
		return $query->result();
	}
	...
	public function list_tokenname($databaseCode,$DepartmenCode)
	{
		...
		$this->db2->select('*');					
		$this->db2->where('DatabaseCode',$databaseCode);
		$this->db2->where('DepartmenCode',$DepartmenCode);
		$query = $this->db2->get('masterdatabase');
		return $query->result();
	}	
