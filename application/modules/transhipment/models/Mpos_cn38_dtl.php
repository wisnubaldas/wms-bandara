...
class Mpos_cn38_dtl extends CI_Model
{
	...
	function insert($insert){
        if($this->db->insert('t_cn38_dtl',$insert)){
            return TRUE;
        }else{
            return FALSE;
        }
    ...
    function update($cn38Code,$update){
        if($this->db->update('t_cn38_dtl',$update,'cn38Code = \''.$cn38Code.'\'')){
            return TRUE;
        }else{
            return FALSE;
        }
    ...
    function delete($cn38Code){
        ...
        if($this->db->delete('t_cn38_dtl',$delete)){
            return TRUE;
        }else{
            return FALSE;
        }
	...
	function select($select=NULL,$awal=0,$akhir=10){
        ...
        $query = $this->db->get('t_cn38_dtl',$akhir,$awal);
        return $query->result();
    }
	
    function view($select=NULL){
        if(! is_null($select))$this->db->like($select);
        $query = $this->db->get('t_cn38_dtl');
