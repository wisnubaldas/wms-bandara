...
class Mpos_cn31 extends CI_Model
{
	...
	function insert($insert){
        if($this->db->insert('t_cn31',$insert)){
            return TRUE;
        }else{
            return FALSE;
        }
    ...
    function update($cn31Code,$update){
        if($this->db->update('t_cn31',$update,'cn31Code = \''.$cn31Code.'\'')){
            return TRUE;
        }else{
            return FALSE;
        }
    ...
    function delete($cn31Code){
        ...
        if($this->db->delete('t_cn31',$delete)){
            return TRUE;
        }else{
            return FALSE;
        }
	...
	function select($select=NULL,$awal=0,$akhir=10){
        ...
        $query = $this->db->get('t_cn31',$akhir,$awal);
        return $query->result();
    }
	
    function view($select=NULL){
        if(! is_null($select))$this->db->like($select);
        $query = $this->db->get('t_cn31');
