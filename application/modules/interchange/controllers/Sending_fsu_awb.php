...
class Sending_fsu_awb extends REST_Controller {
	...
	function index_get() {
        $noid = $this->get('noid');
        if ($noid == '') {
			$data = $this->db2->get('sending_fsu_awb')->result();            
        } else {
	...
	function index_put() {
        ...
        $data = array(	
					...
					'TotallNumberOfPieces'			=> $this->put('TotallNumberOfPieces'),
		...
		$this->db2->where('noid', $noid);
        $update = $this->db2->update('sending_fsu_awb', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
					...
					'TotallNumberOfPieces'			=> $this->post('TotallNumberOfPieces'),
		 ...
		 $insert = $this->db2->insert('sending_fsu_awb', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
