...
class Sending_fwb_9_alsonotify extends REST_Controller {
	...
	function index_get() {
        $id = $this->get('id');
        if ($id == '') {
			$data = $this->db2->get('sending_fwb_9_alsonotify')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db2->where('id', $id);
        $update = $this->db2->update('sending_fwb_9_alsonotify', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db2->insert('sending_fwb_9_alsonotify', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
