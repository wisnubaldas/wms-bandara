<?php
defined('BASEPATH') OR exit('No direct script access allowed');

require APPPATH . 'libraries/REST_Controller.php';

class Sending_ffm_2_manifestheader extends REST_Controller {
	
	function __construct()
	{
		// Construct the parent class
		parent::__construct();
		$this->db2 = $this->load->database('rdinterchangemessage', TRUE);	
	}
	
	function index_get() {
        $id = $this->get('id');
        if ($id == '') {
			$data = $this->db2->get('sending_ffm_2_manifestheader')->result();            
        } else {
            $this->db2->where('id', $id);
            $data = $this->db2->get('sending_ffm_2_manifestheader')->result();
        }
        $this->response($data, 200);
    }
	
	function index_put() {
		$id = $this->put('id');
        $data = array(	
					'MessageLineNo'				=> $this->put('MessageLineNo'),
					'MessageSequenceNumber'		=> $this->put('MessageSequenceNumber'),
					'CarrierCode'				=> $this->put('CarrierCode'),
					'FlightNumber'				=> $this->put('FlightNumber'),
					'DayOfScheduleDeparture'	=> $this->put('DayOfScheduleDeparture'),
					'MonthOfScheduleDeparture'	=> $this->put('MonthOfScheduleDeparture'),
					'AirportCodeOfLoading'		=> $this->put('AirportCodeOfLoading'),
					'AircraftRegistration'		=> $this->put('AircraftRegistration'),
					'MessageKey'				=> $this->put('MessageKey')
					);
					
		$this->db2->where('id', $id);
        $update = $this->db2->update('sending_ffm_2_manifestheader', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
        }
	}
	
	function index_post() {		
        $data = array(		
					'MessageKey'				=> $this->post('MessageKey'),
					'MessageLineNo'				=> $this->post('MessageLineNo'),
					'MessageSequenceNumber'		=> $this->post('MessageSequenceNumber'),
					'CarrierCode'				=> $this->post('CarrierCode'),
					'FlightNumber'				=> $this->post('FlightNumber'),
					'DayOfScheduleDeparture'	=> $this->post('DayOfScheduleDeparture'),
					'MonthOfScheduleDeparture'	=> $this->post('MonthOfScheduleDeparture'),
					'AirportCodeOfLoading'		=> $this->post('AirportCodeOfLoading'),
					'AircraftRegistration'		=> $this->post('AircraftRegistration')
					);
					
		 $insert = $this->db2->insert('sending_ffm_2_manifestheader', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
        }
	}
}