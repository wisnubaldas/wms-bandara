...
class Sending_fsu_tfd extends REST_Controller {
	...
	function index_get() {
        $noid = $this->get('noid');
        if ($noid == '') {
			$data = $this->db2->get('sending_fsu_tfd')->result();            
        } else {
	...
	function index_put() {
        ...
        $data = array(	
					...
					'CarrierCodeOfReceivingCarrier'		=> $this->put('CarrierCodeOfReceivingCarrier'),
					...
					'TransferManifestNumber'			=> $this->put('TransferManifestNumber'),
					'MessageSent'						=> $this->put('MessageSent')
					);
					
		$this->db2->where('noid', $noid);
        $update = $this->db2->update('sending_fsu_tfd', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
					...
					'CarrierCodeOfReceivingCarrier'		=> $this->post('CarrierCodeOfReceivingCarrier'),
					...
					'TransferManifestNumber'			=> $this->post('TransferManifestNumber'),
					'MessageSent'						=> $this->post('MessageSent')
					);
					
		 $insert = $this->db2->insert('sending_fsu_tfd', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
