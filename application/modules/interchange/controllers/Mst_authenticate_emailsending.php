...
class Mst_authenticate_emailsending extends REST_Controller {
	...
	function index_get() {
        ...
        if ($DefaultHost == '') {
			$data = $this->db->get('mst_authenticate_emailsending')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db->where('DefaultHost', $DefaultHost);
        $update = $this->db->update('mst_authenticate_emailsending', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db->insert('mst_authenticate_emailsending', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
