...
class Sending_fsu_dlv extends REST_Controller {
	...
	function index_get() {
        $noid = $this->get('noid');
        if ($noid == '') {
			$data = $this->db2->get('sending_fsu_dlv')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db2->where('noid', $noid);
        $update = $this->db2->update('sending_fsu_dlv', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db2->insert('sending_fsu_dlv', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
