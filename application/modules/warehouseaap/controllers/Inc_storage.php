...
class Inc_storage extends REST_Controller {
	...
	function index_get() {
        $noid = $this->get('noid');
        if ($noid == '') {
			$data = $this->db->get('inc_storage')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db->where('noid', $noid);
        $update = $this->db->update('inc_storage', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db->insert('inc_storage', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
