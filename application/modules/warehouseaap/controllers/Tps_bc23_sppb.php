...
class Tps_bc23_sppb extends REST_Controller {
	...
	function index_get() {
        $noid = $this->get('noid');
        if ($noid == '') {
			$data = $this->db4->get('getbc23_sppb')->result();            
        } else {
	...
	function index_put() {
        ...
        $data = array(	
					...
					'KD_KANTOR_PENGAWAS'	=> $this->put('KD_KANTOR_PENGAWAS'),
					'KD_KANTOR_BONGKAR'		=> $this->put('KD_KANTOR_BONGKAR'),
		...
		$this->db4->where('noid', $noid);
        $update = $this->db4->update('getbc23_sppb', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
					...
					'KD_KANTOR_PENGAWAS'	=> $this->post('KD_KANTOR_PENGAWAS'),
					'KD_KANTOR_BONGKAR'		=> $this->post('KD_KANTOR_BONGKAR'),
		 ...
		 $insert = $this->db4->insert('getbc23_sppb', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
