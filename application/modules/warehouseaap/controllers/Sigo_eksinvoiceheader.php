...
class Sigo_eksinvoiceheader extends REST_Controller {
	...
	function index_get() {
        ...
        if ($InvoiceNumber == '') {
			$data = $this->db->get('sigo_eks_invoiceheader')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db->where('InvoiceNumber', $InvoiceNumber);
        $update = $this->db->update('sigo_eks_invoiceheader', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db->insert('sigo_eks_invoiceheader', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
