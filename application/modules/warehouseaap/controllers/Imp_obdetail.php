...
class Imp_obdetail extends REST_Controller {
	...
	function index_get() {
        $noid = $this->get('noid');
        if ($noid == '') {
			$data = $this->db->get('imp_obdetail')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db->where('noid', $noid);
        $update = $this->db->update('imp_obdetail', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db->insert('imp_obdetail', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
