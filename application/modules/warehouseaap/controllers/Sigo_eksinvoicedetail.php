...
class Sigo_eksinvoicedetail extends REST_Controller {
	
	function __construct()
	{
		// Construct the parent class
		parent::__construct();
		$this->db = $this->load->database('rdwarehouse_aap', TRUE);
	}
	
	function index_get() {
        $InvoiceNumber = $this->get('InvoiceNumber');
        if ($InvoiceNumber == '') {
			$data = $this->db->get('sigo_eks_invoicedetail')->result();            
        } else {
	...
	function index_put() {
        ...
        $update = $this->db->update('sigo_eks_invoicedetail', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(	
		 ...
		 $insert = $this->db->insert('sigo_eks_invoicedetail', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
