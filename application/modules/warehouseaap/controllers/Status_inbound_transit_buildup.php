...
class Status_inbound_transit_buildup extends REST_Controller {
	...
	function index_get() {
        $id_ = $this->get('id_');
        if ($id_ == '') {
			$data = $this->db->get('td_inbound_transit_buildup')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db->where('id_', $id_);
        $update = $this->db->update('td_inbound_transit_buildup', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db->insert('td_inbound_transit_buildup', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
