...
class Status_outbond_delivery_staging extends REST_Controller {
	...
	function index_get() {
        $id_ = $this->get('id_');
        if ($id_ == '') {
			$data = $this->db->get('td_outbond_delivery_staging')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db->where('id_', $id_);
        $update = $this->db->update('td_outbond_delivery_staging', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db->insert('td_outbond_delivery_staging', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
