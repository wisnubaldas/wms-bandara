...
class Eks_sticker extends REST_Controller {
	...
	function index_get() {
        $noid = $this->get('noid');
        if ($noid == '') {
			$data = $this->db->get('eks_sticker')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db->where('noid', $noid);
        $update = $this->db->update('eks_sticker', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db->insert('eks_sticker', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
