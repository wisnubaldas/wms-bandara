...
class Dbreceipt_header extends REST_Controller {
	...
	function index_put() {
        ...
        $data = array(	
					...
					'receipt_real'			=> $this->put('receipt_real'),						
		...
		$this->db6->where('_id', $_id);
        $update = $this->db6->update('dbreceipt_header', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
					...
					'receipt_real'			=> $this->post('receipt_real'),					
		 ...
		 $insert = $this->db6->insert('dbreceipt_header', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
