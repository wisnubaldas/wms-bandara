...
class Dbreceipt_detail extends REST_Controller {
	...
	function index_get() {
        $_id = $this->get('_id');
        if ($_id == '') {
			$data = $this->db6->get('dbreceipt_detail')->result();            
        } else {
	...
	function index_put() {
        ...
        $data = array(	
					...
					'FlagOfType'		=> $this->put('FlagOfType'),	
		...
		$this->db6->where('_id', $_id);
        $update = $this->db6->update('dbreceipt_detail', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
					...
					'FlagOfType'		=> $this->post('FlagOfType'),
					'_created_by'		=> $this->post('_created_by')
					);
					
		 $insert = $this->db6->insert('dbreceipt_detail', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
