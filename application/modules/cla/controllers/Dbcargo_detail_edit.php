...
class Dbcargo_detail_edit extends REST_Controller {
	...
	function index_get() {
        ...
        if ($_id == '') {
			$data = $this->db6->get('dbcargo_detail_edit')->result();            
        } else {
	...
	function index_put() {
		...
		$this->db6->where('_id', $_id);
        $update = $this->db6->update('dbcargo_detail_edit', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db6->insert('dbcargo_detail_edit', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
