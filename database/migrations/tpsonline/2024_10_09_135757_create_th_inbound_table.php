...
class CreateThInboundTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('th_inbound', function (Blueprint $table) {
            $table->bigInteger('id_')->primary();
            $table->string('tps', 4)->nullable()->index('tps');
            $table->enum('gate_type', ['import', 'incoming', 'transit']);
            $table->string('waybill_smu', 15)->nullable()->index('waybill_smu');
            ...
            $table->boolean('full_check')->default(0);
        });
    }
    ...
    {
        Schema::dropIfExists('th_inbound');
    }
}
