    ...
    protected function schemax($data,$INC_OUT)
    {
        ...
        foreach ($data as $v) {
            $datax = [
                    ...
                    'CARGO_CHG' => (float)$v->WarehouseFee,
                    'KADE' => (float)$v->TotalAssistancyFee,
                    'TOTAL_PENDAPATAN_TANPA_PPN' => (float)$v->SubTotalFee,
                    'TOTAL_PENDAPATAN_DENGAN_PPN' => (float)$v->GrandTotalFee,
                    'PJT_HANDLING_FEE' => 0,
                    'RUSH_HANDLING_FEE' => 0,
                    'RUSH_SERVICE_FEE' => 0,
                    'TRANSHIPMENT_FEE' => 0,
                    'ADMINISTRATION_FEE' => (float)$v->AdministrationFee,
                    'DOCUMENTS_FEE' => (float)$v->DocumentFee,
                    'PECAH_PU_FEE' => (float)$v->Totalpecahpos,
                    'COOL_COLD_STORAGE_FEE' => (float)$v->ColdStorageFee,
                    'STRONG_ROOM_FEE' => (float)$v->StrongRoomFee,
                    'AC_ROOM_FEE' => (float)$v->AirConditioningFee,
                    'DG_ROOM_FEE' => (float)$v->DangerousRoomFee,
                    'AVI_ROOM_FEE' => 0,
                    ...
                    'HAWB_FEE' => 0,
                    'HAWB_MAWB_FEE' => 0,
                    'CSC_FEE' => (float)$v->OtherFee,
                    'ENVIROTAINER_ELEC_FEE' => 0,
                    'ADDITIONAL_COSTS' => (float)$v->AirportContriFee,
                    'NAWB_FEE' => 0,
                    'BARCODE_FEE' => 0,
                    ...
                    'MATERAI_FEE' => (float)$v->StampFee,
                    'PPN_FEE' => (float)$v->TaxFee,
