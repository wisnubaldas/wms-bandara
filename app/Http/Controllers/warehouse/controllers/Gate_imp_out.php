...
class Gate_imp_out extends REST_Controller {
	...
	function index_get() {
        ...
        if ($id_kms == '') {
			$data = $this->db4->get('get_imp_out')->result();            
        } else {
	...
	function index_put() {
        ...
        $data = array(		
					...
					'date_create'			=> $this->put('date_create'),
					'date_update'			=> $this->put('date_update'),
		...
		$this->db4->where('id_kms', $id_kms);
        $update = $this->db4->update('get_imp_out', $data);
        if ($update) {
            $this->response($data, 200);
        } else {
	...
	function index_post() {		
        $data = array(		
		 ...
		 $insert = $this->db4->insert('get_imp_out', $data);
        if ($insert) {
            $this->response($data, 200);
        } else {
            $this->response(array('status' => 'fail', 502));
